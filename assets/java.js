var giff = ["Pizza", "High", "Beer", "Barbie", "Queen", "Joint", "Moon", "Tattoo", "Angry", "Rainbow", "RuPaul", "Inspired", "Life", "Earth", "Sea", "Glitter", "Fabulous", "Love"];

window.onload = function () {

    function renderButtons() {
        $("#buttons-display").empty();
        for (var i = 0; i < giff.length; i++) {

            var a = $("<button>");
            a.addClass("emotion btn btn-secondary m-1");
            a.attr("data-search", giff[i]);
            a.text(giff[i]);
    ...
    $("#add-emotion").on("click", function (event) {
        ...
        giff.push(emotion);
        renderButtons();
        console.log("Index length: " + giff.length);
        getGiphy();
    });
